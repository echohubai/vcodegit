...
(function Slider($,window,document,undefined) {
    function SliderUnlock(elm, options, success){
        ...
        //滑块背景
        //图片碎片所处位置
        me.piece = me.elm.find('#puzzle-the-piece');
        me.sliderBg = me.elm.find('#slider_bg');
        //succes
        me.success = success;
		//记录鼠标轨迹
    ...
    SliderUnlock.prototype.init = function () {
        ...
        me.elm.find("#label").on("mousedown", function (event) {
			...
			//var startTime=new Date();//记录鼠标点下时时间
            me.handerMove(event);
    ...
    SliderUnlock.prototype.handerIn = function () {
        ...
        var passTime = endTime.getTime()-me.startTime.getTime();

        me.swipestart = true;
        me.min = 0;
        me.max = me.elm.width();
        me.mtrace.push("1"+":"+event.clientX+':'+event.clientY+':'+passTime);
		
    };
    /**
     * 鼠标/手指移出
     */
    SliderUnlock.prototype.handerUp = function () {
        $('#label').next('#labelTip').text("验证中").css({'color': '#fff'});
        var me = this;
        var endTime = new Date();
        var passTime = endTime.getTime()-me.startTime.getTime();
        me.mtrace.push("2"+":"+event.clientX+':'+event.clientY+':'+passTime);
        if(me.mtrace.length>=500)
        {
            alert("验证失败");
            failReset(0);
            $('#label').next('#labelTip').text("验证失败！请重试").css({'color': '#fff'});
            me.mtrace=[];
           /* pictureRandByFail(token, AppID);*/
        }

      else {
            var str = me.mtrace.join("-");
            var encstr = Encrypt(str);

            sendTrace(encstr, me);

            console.log("enctrace:" + encstr);
            console.log("trace:" + str);
            console.log("length:" + me.mtrace.length);
            me.mtrace=[];
        }
    ...
    SliderUnlock.prototype.handerMove = function (event,type) {
        ...
        if (me.swipestart) {
            ...
            if (type == "mobile") {
				...
				me.mtrace.push("3"+":"+event.clientX+':'+event.clientY+':'+passTime);
    ...
    function sendTrace(trace){
        ...
        $.ajax({
            ...
            success: function (result) {
               ...
               if(flag) {
                   alert("验证成功");
                   $('#label').unbind().next('#labelTip').text("验证成功！").css({'color': '#fff'});
                   validateTest = data.validate;
                   $('#Validate').val(validateTest);
               }
               else {
                   var m = "failed"
                   var info = data.msg;
                   if (info==m ){
                       alert("验证失败次数太多，请刷新页面重试");
            ...
            complete : function(XMLHttpRequest,status){ //请求完成后最终执行参数
                if(status=='error'){
                    $('#label').unbind().next('#labelTip').text("验证失败！").css({'color': '#fff'});
                }
            }
...
* */
  function Encrypt(word){
        var token = document.getElementById("Token").value;
		 var key = CryptoJS.enc.Utf8.parse(token.substring(10,26));
...
function showPic(){
    ...
    document.getElementById("hidden").style.top = y;
   // document.getElementById("puzzle-without-piece").innerHTML = "<img src=\"" + sUrl + "\">";
...
function  failReset(index) {
      $("#slider_bg").animate({'width': 0},200);
      $("#label").animate({left:index},200).next("#labelTip").animate({opacity:1},200);
    /*  $("#slider_bg").css("width",0);*/
      $("#puzzle-the-piece").css("left","0px");
     /* $("#label").css("left","0px");*/
